...
def read_remaining_equity_sections(ws, first_total_row, col_map):
    ...
    For Cash and Bank / Margin, also capture their Sub Total / Total rows so the same
    Trial Balance calculated Market/Fair Value can be checked against those rows.
    """
    name_col = col_map["Name of the Instrument"]
    ...
    for row in range(first_total_row + 1, ws.max_row + 1):
        ...
        if label_lower in {"sub total", "total"}:
            if current_section in TRIAL_BALANCE_ACCOUNT_CODES:
                total_rec = row_values_from_sebi(ws, row, col_map)
                total_rec["Total Label"] = label
    ...
    return section_records, section_total_rows
...
def compare_trial_balance_section(ws, scheme_code, section_name, sebi_records, trial_df, all_discrepancies, col_map, total_rows=None):
    ...
    # Check Sub Total / Total Market/Fair Value for Cash and Bank / Margin only.
    # We use the same Trial Balance calculated value already used for the section row.
    # % to Net Assets for these Sub Total / Total rows is not checked here.
    for total_row in total_rows or []:
        total_label = clean_text(total_row.get("Total Label")) or clean_text(total_row.get("Name of the Instrument"))
        total_sebi_value = market_value_for_compare(total_row.get(market_col))

        if trial_value != total_sebi_value:
            add_discrepancy(
                all_discrepancies,
                scheme_code,
                f"Remaining Section - {section_name}",
                ...
                trial_value,
                total_sebi_value,
                f"{total_label} value mismatch",
                total_row.get("Excel Row"),
            )
            highlight_cell(
                ws,
                total_row.get("Excel Row"),
                col_map.get(market_col),
                HIGHLIGHT_FILL,
            )
...
def compare_remaining_equity_sections(ws, scheme_code, master_df, trial_df, all_discrepancies, sebi):
    ...
    section_records, section_total_rows = read_remaining_equity_sections(ws, first_total_row, col_map)
    ...
    for section_name, records in section_records.items():
        if section_name == "TREPS / Reverse Repo":
            ...
            compare_trial_balance_section(
                ...
                all_discrepancies,
                col_map,
                section_total_rows.get(section_name, []),
            )
