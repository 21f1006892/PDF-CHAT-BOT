...
YIELD_MASTER_FILES = [
    "310126-T-BILL_FM 1(1).xls",
    "310126-CPCD_FM 1(1).xls",
    "310126-Bonds_FM_PTC 1(1).xls",
    "310126-Bonds_FM_IRS 1(1).xls",
    "310126-Bonds_FM 1(1).xls",
]
YIELD_MASTER_CONVERTED_DIR = "converted_yield_masters"
OUTPUT_FILE = "SEBI_Monthly_Portfolio_Equity_Debt_Reconciled_From_Correct_Base.xlsx"
...
# This is not a real SEBI section name. It is only used internally for the
# Sub Total row that belongs to Margin + Cash inside the parent Others section.
CASH_MARGIN_TOTAL_SECTION = "Cash and Bank + Margin (Future and Options)"
...
DEBT_REVERSE_REPO_COMPARE_COLS = [
    "Market/Fair Value\n(Rs.in Lacs)",
    "% to Net Assets",
    "Yield",
]
...
def read_remaining_equity_sections(ws, first_total_row, col_map):
    ...
    Important structure:
    - Others is a parent section.
    - Margin (Future and Options) and Cash and Bank are rows inside Others.
    - The Sub Total after Cash and Bank belongs to Margin + Cash.
    - TREPS / Reverse Repo is another child block inside Others.
    - The Total after TREPS / Reverse Repo belongs to the full Others section.
    """
    name_col = col_map["Name of the Instrument"]
    current_parent_section = None
    current_section = None
    last_subtotal_section = None
    ...
    for row in range(first_total_row + 1, ws.max_row + 1):
        ...
        if label == "Others":
            current_parent_section = "Others"
            current_section = None
            last_subtotal_section = None
            continue

        # New section / child section / value row.
        if label in REMAINING_EQUITY_SECTIONS:
            effective_section = get_remaining_effective_section(label, current_section)

            # Margin and Cash are value rows inside Others, not separate sections.
            # Their following Sub Total is for Margin + Cash together.
            if label in TRIAL_BALANCE_ACCOUNT_CODES:
                section_records.setdefault(label, []).append(row_values_from_sebi(ws, row, col_map))
                section_parents[label] = current_parent_section
                current_section = CASH_MARGIN_TOTAL_SECTION
                section_parents[CASH_MARGIN_TOTAL_SECTION] = current_parent_section
                last_subtotal_section = None
                continue

            current_section = effective_section
            section_parents.setdefault(current_section, current_parent_section)
            last_subtotal_section = None
            continue

        if label_lower == "sub total":
            if current_section:
                total_rec = row_values_from_sebi(ws, row, col_map)
                total_rec["Row Label"] = "Sub Total"
                section_total_rows.setdefault(current_section, []).append(total_rec)
                last_subtotal_section = current_section
            continue

        if label_lower == "total":
            total_rec = row_values_from_sebi(ws, row, col_map)
            total_rec["Row Label"] = "Total"

            # Inside Others, the Total after TREPS is the Total of the full Others section,
            # not only TREPS.
            if current_parent_section == "Others" and last_subtotal_section == "TREPS / Reverse Repo":
                section_total_rows.setdefault("Others", []).append(total_rec)
                current_section = None
                last_subtotal_section = None
    ...
    return section_records, section_total_rows, section_parents
...
def calculate_cash_margin_percent(market_value, grand_total):
    mv = to_decimal(market_value)
    gt = to_decimal(grand_total)

    if mv is None or gt is None or gt == 0:
        return None

    return (mv * Decimal("100")) / gt
...
def compare_trial_balance_section(ws, scheme_code, section_name, sebi_records, trial_df, all_discrepancies, col_map):
    ...
    for row in range(sebi_row.get("Excel Row"), ws.max_row + 1):
        label = clean_text(ws.cell(row, name_col).value).casefold()

        if label == "grand total":
            grand_total_value = ws.cell(
                row,
                col_map[market_col]
            ).value
            break
    #--------- mv (custom logic for Cash/Margin only) -------------------
    mv = calculate_cash_margin_percent(
        sebi_value,
        grand_total_value
    )

    #--------- for sv -----------------------
    sv = percent_value_for_compare(
        sebi_row.get(pct_col),
        "SEBI"
    )

    #----------- Final Comparison --------------------
    if mv is not None and sv is not None:
        if not (round_to_2(mv)) == (round_to_2(sv)):
            add_discrepancy(
                all_discrepancies,
                scheme_code,
                f"Remaining Section - {section_name}",
...
def expected_master_section_totals(master_df, scheme_code, section_name):
    master_records, _, _ = prepare_master_remaining_table(master_df, scheme_code, section_name)
    market_total = add_decimals(r.get("Market/Fair Value\n(Rs.in Lacs)") for r in master_records)
    percent_total = add_decimals(r.get("% to Net Assets") for r in master_records)
    return market_total, percent_total


def expected_treps_totals(master_df, scheme_code):
    master_records = prepare_master_treps_records(master_df, scheme_code)
    market_total = add_decimals(r.get("Market/Fair Value\n(Rs.in Lacs)") for r in master_records)
    percent_total = add_decimals(r.get("% to Net Assets") for r in master_records)
    return market_total, percent_total
...
def expected_cash_margin_market_total(trial_df, scheme_code):
    values = []
    for section_name in TRIAL_BALANCE_ACCOUNT_CODES:
        val = trial_balance_market_value(trial_df, scheme_code, section_name)
        if val is not None:
            values.append(val)
    return add_decimals(values)


def expected_others_market_total(master_df, trial_df, scheme_code, section_records, section_parents):
    values = []

    for section_name, parent in section_parents.items():
        if parent != "Others":
            continue

        if section_name == CASH_MARGIN_TOTAL_SECTION:
            continue

        if section_name in TRIAL_BALANCE_ACCOUNT_CODES:
            val = trial_balance_market_value(trial_df, scheme_code, section_name)
            if val is not None:
                values.append(val)
            continue
        ...
        if section_name == "TREPS / Reverse Repo":
            market_total, _ = expected_treps_totals(master_df, scheme_code)
            if market_total is not None:
                values.append(market_total)
            continue

        if section_name in REMAINING_SECTION_SECURITY_TYPES:
            market_total, _ = expected_master_section_totals(master_df, scheme_code, section_name)
            if market_total is not None:
                values.append(market_total)
...
def compare_total_market_value(ws, scheme_code, section_name, total_row, expected_market_value, all_discrepancies, col_map):
    market_col = "Market/Fair Value\n(Rs.in Lacs)"
    sebi_market_value = market_value_for_compare(total_row.get(market_col))
    row_label = total_row.get("Row Label", "Total")

    if expected_market_value is None:
        add_discrepancy(
            all_discrepancies,
            scheme_code,
            f"Remaining Section - {section_name} {row_label}",
            "",
            row_label,
            market_col,
            None,
            sebi_market_value,
            "Could not calculate expected Market/Fair Value",
            total_row.get("Excel Row"),
        )
        return

    if expected_market_value != sebi_market_value:
        add_discrepancy(
            all_discrepancies,
            scheme_code,
            f"Remaining Section - {section_name} {row_label}",
        ...
        highlight_cell(ws, total_row.get("Excel Row"), col_map.get(market_col), HIGHLIGHT_FILL)


def compare_total_percent_value(ws, scheme_code, section_name, total_row, expected_percent_value, all_discrepancies, col_map):
    pct_col = "% to Net Assets"
    sebi_percent_value = percent_value_for_compare(total_row.get(pct_col), "SEBI")
    sebi_percent_compare = round_to_2(sebi_percent_value) if sebi_percent_value is not None else None
    expected_percent_compare = round_to_2(expected_percent_value) if expected_percent_value is not None else None
    row_label = total_row.get("Row Label", "Total")

    if expected_percent_compare is None:
        add_discrepancy(
            all_discrepancies,
            scheme_code,
            f"Remaining Section - {section_name} {row_label}",
            ...
            "Could not calculate expected % to Net Assets",
            total_row.get("Excel Row"),
        )
        return

    if expected_percent_compare != sebi_percent_compare:
        add_discrepancy(
            all_discrepancies,
            scheme_code,
            f"Remaining Section - {section_name} {row_label}",
        ...
        highlight_cell(ws, total_row.get("Excel Row"), col_map.get(pct_col), HIGHLIGHT_FILL)


def compare_remaining_total_rows(ws, scheme_code, section_total_rows, section_records, section_parents, master_df, trial_df, all_discrepancies, col_map):
    for section_name, total_rows in section_total_rows.items():
        compare_percent = True
        expected_market = None
        expected_percent = None

        if section_name == CASH_MARGIN_TOTAL_SECTION:
            expected_market = expected_cash_margin_market_total(trial_df, scheme_code)
            compare_percent = False

        elif section_name == "Others":
            expected_market = expected_others_market_total(master_df, trial_df, scheme_code, section_records, section_parents)
            compare_percent = False

        elif section_name == "TREPS / Reverse Repo":
            expected_market, expected_percent = expected_treps_totals(master_df, scheme_code)

        elif section_name in REMAINING_SECTION_SECURITY_TYPES:
            expected_market, expected_percent = expected_master_section_totals(master_df, scheme_code, section_name)

        else:
            continue

        for total_row in total_rows:
            compare_total_market_value(ws, scheme_code, section_name, total_row, expected_market, all_discrepancies, col_map)
            if compare_percent:
                compare_total_percent_value(ws, scheme_code, section_name, total_row, expected_percent, all_discrepancies, col_map)
...
def compare_remaining_equity_sections(ws, scheme_code, master_df, trial_df, all_discrepancies, sebi):
    ...
    section_records, section_total_rows, section_parents = read_remaining_equity_sections(ws, first_total_row, col_map)
...
def get_debt_header_map(ws):
    for row in range(1, ws.max_row + 1):
        values = [canonical_header(ws.cell(row, col).value) for col in range(1, ws.max_column + 1)]
        if "Name of the Instrument" in values and "ISIN" in values and "Yield" in values:
            header_row = row
            col_map = {}
            for col in range(1, ws.max_column + 1):
                header = canonical_header(ws.cell(row, col).value)
                if header == "Industry^ / Rating":
                    header = "Rating"
                if header in DEBT_SEBI_COLUMNS:
                    col_map[header] = col
            missing = [c for c in DEBT_SEBI_COLUMNS if c not in col_map]
            if missing:
                available = [norm_header(ws.cell(row, col).value) for col in range(1, ws.max_column + 1) if ws.cell(row, col).value]
                raise ValueError(f"Missing required DEBT SEBI columns {missing}. Available headers: {available}")
            return header_row, col_map
    raise ValueError(f"Debt header row not found in sheet {ws.title}")


def is_debt_reverse_repo_label(label):
    name = normalize_key(label)
    return name in {
        normalize_key("Reverse Repo"),
        normalize_key("Clearing Corporation of India Limited"),
    }


def debt_sebi_reverse_repo_type_key(value):
    name = normalize_key(value)
    if name == normalize_key("Reverse Repo"):
        return "REPO"
    if name == normalize_key("Clearing Corporation of India Limited"):
...
def debt_master_reverse_repo_type_key(value):
    return treps_master_type_key(value)


def read_sebi_debt_rows(ws):
    header_row, col_map = get_debt_header_map(ws)
    normal_records = []
    normal_by_isin = {}
    reverse_repo_records = []
    in_reverse_repo_section = False

    for row in range(header_row + 1, ws.max_row + 1):
        label = clean_text(ws.cell(row, col_map["Name of the Instrument"]).value)
        label_lower = label.casefold()

        if label_lower == "treps / reverse repo":
            in_reverse_repo_section = True
            continue

        if label_lower in {"sub total", "total", "grand total"}:
            if in_reverse_repo_section:
                in_reverse_repo_section = False
            continue
        ...
        isin = normalize_isin(rec.get("ISIN"))

        if in_reverse_repo_section or is_debt_reverse_repo_label(label):
            if is_debt_reverse_repo_label(label):
                rec["Reverse Repo Type"] = debt_sebi_reverse_repo_type_key(label)
                reverse_repo_records.append(rec)
            continue
        ...
        normal_by_isin[isin] = rec

    return {
        "header_row": header_row,
        "col_map": col_map,
        "records": normal_records,
        "row_by_isin": normal_by_isin,
        "reverse_repo_records": reverse_repo_records,
    }


def prepare_master_debt_rows(master_df, scheme_code):
    table = master_df[master_df["Client Code"] == scheme_code].copy()
    if table.empty:
        return [], {}
    ...
    records = []
    for _, row in table.iterrows():
        if debt_master_reverse_repo_type_key(row.get("Security Type Name")):
            continue
    ...
    by_isin = {normalize_isin(r.get("ISIN")): r for r in records if normalize_isin(r.get("ISIN"))}
    return records, by_isin


def prepare_master_debt_reverse_repo_records(master_df, scheme_code):
    table = master_df[master_df["Client Code"] == scheme_code].copy()
    records = []

    for _, row in table.iterrows():
        type_key = debt_master_reverse_repo_type_key(row.get("Security Type Name"))
        if not type_key:
            continue

        rec = {
            "Reverse Repo Type": type_key,
            "Name of the Instrument": "Reverse Repo" if type_key == "REPO" else "Clearing Corporation of India Limited",
            "ISIN": row.get("ISIN"),
            "Market/Fair Value\n(Rs.in Lacs)": converted_market_value_from_source(row.get("Total Market Value (Rs.)")),
            "% to Net Assets": row.get("% to Net assests"),
...
def resolve_xls_to_xlsx_for_reading(xls_file, output_dir):
    if xls_file.lower().endswith(".xlsx"):
        return xls_file

    os.makedirs(output_dir, exist_ok=True)
    base_name = os.path.splitext(os.path.basename(xls_file))[0] + ".xlsx"
    xlsx_path = os.path.join(output_dir, base_name)

    if os.path.exists(xlsx_path):
        return xlsx_path

    source_path = xls_file if os.path.exists(xls_file) else os.path.join(".", xls_file)

    for command in ["libreoffice", "soffice"]:
        try:
            subprocess.run(
                [command, "--headless", "--convert-to", "xlsx", source_path, "--outdir", output_dir],
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.PIPE,
            )
...
def read_yield_file(file_path):
    records = []
    try_paths = [file_path]
    if file_path.lower().endswith(".xls"):
        try_paths.append(resolve_xls_to_xlsx_for_reading(file_path, YIELD_MASTER_CONVERTED_DIR))
        ...
        try:
            ...
            for sheet_name in xls.sheet_names:
                ...
                for idx in range(len(raw)):
                    row_values = [clean_text(v) for v in raw.iloc[idx].tolist()]
                    if "ISIN Number" in row_values and "Aggregated Yield" in row_values:
                        header_row = idx
                        break
                ...
                if "ISIN Number" not in data.columns or "Aggregated Yield" not in data.columns:
                    continue

                for _, row in data.iterrows():
                    isin = normalize_isin(row.get("ISIN Number"))
                    agg_yield = row.get("Aggregated Yield")
                    if isin and to_decimal(agg_yield) is not None:
                        records.append(
                            {
                                "ISIN": isin,
                                "Aggregated Yield": agg_yield,
                                "Yield Source File": os.path.basename(path),
                                "Yield Source Sheet": sheet_name,
                            }
                        )
            return records
    ...
    raise RuntimeError(f"Could not read yield master file {file_path}: {last_error}")


def load_yield_master_lookup(required=True):
    lookup = {}
    duplicates = defaultdict(list)
    missing_files = []

    for file_name in YIELD_MASTER_FILES:
        possible_paths = [
            file_name,
        ...
        for rec in read_yield_file(file_path):
            isin = rec["ISIN"]
            if isin not in lookup:
                lookup[isin] = rec
            else:
                duplicates[isin].append(rec)

    if required and missing_files:
        raise FileNotFoundError(
            "Yield master file(s) missing. Debt Yield check cannot run without these file(s): "
            + ", ".join(missing_files)
        )

    if required and not lookup:
        raise RuntimeError(
            "No yield records were loaded from the yield master files. "
            "Check that the files contain columns 'ISIN Number' and 'Aggregated Yield'."
        )

    return lookup, duplicates


def debt_yield_for_compare(value):
    dec_value = to_decimal(value)
    if dec_value is None:
        return None
    return round_to_2(dec_value * Decimal("100"))


def display_debt_yield(value):
    val = debt_yield_for_compare(value)
    return "" if val is None else format(val, "f")


def display_debt_value(value, column_name):
    if column_name == "Yield":
        return display_debt_yield(value)
    if column_name == "Market/Fair Value\n(Rs.in Lacs)":
        val = market_value_for_compare(value)
        return "" if val is None else format(val, "f")
    if column_name == "% to Net Assets":
        return value
    if column_name == "Quantity":
        val = round_to_2(to_decimal(value))
        return "" if val is None else format(val, "f")
    return value


def same_debt_value(master_value, sebi_value, column_name):
    if column_name == "Yield":
        mv = debt_yield_for_compare(master_value)
        sv = debt_yield_for_compare(sebi_value)
        if mv is None and sv is None:
            return True
        if mv is None or sv is None:
            return False
...
def debt_reverse_repo_compare_key(row, source):
    market_value = market_value_for_compare(row.get("Market/Fair Value\n(Rs.in Lacs)"))
    percent_value = percent_value_for_compare(row.get("% to Net Assets"), source)
    percent_value = round_to_2(percent_value) if percent_value is not None else None
    return market_value, percent_value
...
def compare_debt_reverse_repo_section(ws, scheme_code, sebi_records, master_df, yield_lookup, all_discrepancies, col_map):
    master_records = prepare_master_debt_reverse_repo_records(master_df, scheme_code)

    master_buckets = defaultdict(list)
    for rec in master_records:
        key = (rec.get("Reverse Repo Type"),) + debt_reverse_repo_compare_key(rec, "MASTER")
        master_buckets[key].append(rec)

    matched_master_ids = set()

    def compare_reverse_repo_yield(sebi_row, master_row):
        isin = normalize_isin(sebi_row.get("ISIN")) or normalize_isin(master_row.get("ISIN"))
        yield_master_row = yield_lookup.get(isin) if isin else None
        if yield_master_row is None:
            return

        sebi_yield = sebi_row.get("Yield")
        if is_blank_cell(sebi_yield):
            return

        master_yield = yield_master_row.get("Aggregated Yield")
        if not same_debt_value(master_yield, sebi_yield, "Yield"):
            add_discrepancy(
                all_discrepancies,
                scheme_code,
                "Debt Reverse Repo Row",
                isin,
                sebi_row.get("Name of the Instrument"),
                "Yield",
                display_debt_yield(master_yield),
                display_debt_yield(sebi_yield),
                "Value mismatch",
                sebi_row.get("Excel Row"),
            )
            highlight_cell(ws, sebi_row.get("Excel Row"), col_map.get("Yield"), HIGHLIGHT_FILL)

    for sebi_row in sebi_records:
        sebi_type = sebi_row.get("Reverse Repo Type") or debt_sebi_reverse_repo_type_key(sebi_row.get("Name of the Instrument"))
        sebi_isin = normalize_isin(sebi_row.get("ISIN"))
        key = (sebi_type,) + debt_reverse_repo_compare_key(sebi_row, "SEBI")

        available_master_rows = [r for r in master_buckets.get(key, []) if id(r) not in matched_master_ids]
        if available_master_rows:
            master_row = available_master_rows[0]
            matched_master_ids.add(id(master_row))
            compare_reverse_repo_yield(sebi_row, master_row)
            continue

        # If SEBI has one blank-ISIN row representing the total of many REPO/CBL rows,
        # compare against aggregated unmatched master rows of the same type.
        if not sebi_isin:
            same_type_master_rows = [
                r for r in master_records
                if r.get("Reverse Repo Type") == sebi_type and id(r) not in matched_master_ids
            ]
            expected_market = add_decimals(r.get("Market/Fair Value\n(Rs.in Lacs)") for r in same_type_master_rows)
            expected_percent = add_decimals(r.get("% to Net Assets") for r in same_type_master_rows)
            sebi_market, sebi_percent = debt_reverse_repo_compare_key(sebi_row, "SEBI")

            if expected_market == sebi_market and expected_percent == sebi_percent:
                for master_row in same_type_master_rows:
                    matched_master_ids.add(id(master_row))
                # No Yield comparison for aggregate rows because there is no single ISIN.
                continue
            ...
            add_discrepancy(
                ...
                sebi_row.get("Name of the Instrument"),
                "Market/Fair Value + % to Net Assets",
                f"Market/Fair Value={format_decimal_for_index(expected_market)}, % to Net Assets={format_decimal_for_index(expected_percent)}%",
                f"Market/Fair Value={format_decimal_for_index(sebi_market)}, % to Net Assets={format_decimal_for_index(sebi_percent)}%",
                "Value mismatch",
                sebi_row.get("Excel Row"),
            )
            highlight_cell(ws, sebi_row.get("Excel Row"), col_map.get("Market/Fair Value\n(Rs.in Lacs)"), HIGHLIGHT_FILL)
    ...
    for master_row in master_records:
        ...
        market_value, percent_value = debt_reverse_repo_compare_key(master_row, "MASTER")
        add_discrepancy(
            all_discrepancies,
            scheme_code,
            "Debt Reverse Repo Row",
...
def compare_debt_scheme(ws, scheme_code, master_df, yield_lookup, all_discrepancies):
    sebi = read_sebi_debt_rows(ws)
    master_records, master_by_isin = prepare_master_debt_rows(master_df, scheme_code)
    sebi_by_isin = sebi["row_by_isin"]
    col_map = sebi["col_map"]
    ...
    for isin in all_isins:
        ...
        if master_row is None:
            add_discrepancy(
                ...
                "Debt Security Row",
                isin,
                sebi_row.get("Name of the Instrument") if sebi_row else "",
                "Entire Row",
                None,
                "Present",
                "Master workbook does not have this entire row",
                sebi_row.get("Excel Row") if sebi_row else None,
            )
            continue
        ...
        yield_master_row = yield_lookup.get(isin)

        for col in DEBT_NORMAL_COMPARE_COLS:
            if col == "Yield":
                if yield_master_row is None:
                    continue

                master_value = yield_master_row.get("Aggregated Yield")
                sebi_value = sebi_row.get("Yield")

                if is_blank_cell(sebi_value):
                    continue

                if not same_debt_value(master_value, sebi_value, "Yield"):
                    add_discrepancy(
                        all_discrepancies,
                        scheme_code,
                        "Debt Security Row",
                        ...
                        sebi_row.get("Name of the Instrument"),
                        "Yield",
                        display_debt_yield(master_value),
                        display_debt_yield(sebi_value),
                        "Value mismatch",
                        sebi_row.get("Excel Row"),
                    )
                    highlight_cell(ws, sebi_row.get("Excel Row"), col_map.get("Yield"), HIGHLIGHT_FILL)
            ...
            if not same_debt_value(master_row.get(col), sebi_row.get(col), col):
                add_discrepancy(
                    all_discrepancies,
                    scheme_code,
                    "Debt Security Row",
                    ...
                    master_row.get("Name of the Instrument") or sebi_row.get("Name of the Instrument"),
                    col,
                    display_debt_value(master_row.get(col), col),
                    display_debt_value(sebi_row.get(col), col),
                    "Value mismatch",
                    sebi_row.get("Excel Row"),
                )
                highlight_cell(ws, sebi_row.get("Excel Row"), col_map.get(col), HIGHLIGHT_FILL)

    compare_debt_reverse_repo_section(
        ws,
        scheme_code,
        sebi.get("reverse_repo_records", []),
        master_df,
        yield_lookup,
        all_discrepancies,
        col_map,
...
def main():
    ...
    yield_lookup, yield_duplicates = load_yield_master_lookup()
    ...
    for sheet_name in wb.sheetnames:
        ...
        try:
            if scheme_type == "EQUITY":
                ...
                compare_debt_scheme(ws, sheet_name, master_df, yield_lookup, discrepancies)
