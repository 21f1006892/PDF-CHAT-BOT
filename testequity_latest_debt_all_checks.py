...
def load_yield_master_lookup():
    """Load all uploaded yield master files, keyed by ISIN."""
    lookup = {}
    duplicates = defaultdict(list)

    for file_name in YIELD_MASTER_FILES:
        file_path = file_name if os.path.exists(file_name) else os.path.join(os.getcwd(), file_name)
        if not os.path.exists(file_path):
            # Also try /mnt/data when running from another folder.
            alt_path = os.path.join("/mnt/data", file_name)
