...
OUTPUT_FILE = "SEBI_Monthly_Portfolio_Equity_Debt_Yield_Reconciled_Latest.xlsx"
...
def get_debt_header_map(ws):
    """Find the SEBI debt header row and map debt columns."""
    for row in range(1, ws.max_row + 1):
        values = [canonical_header(ws.cell(row, col).value) for col in range(1, ws.max_column + 1)]
...
def read_sebi_debt_rows(ws):
    """Read all debt security rows that have an ISIN from the SEBI debt sheet."""
...
def prepare_master_debt_rows(master_df, scheme_code):
    """Prepare master rows for a debt scheme, keyed by ISIN."""
    table = master_df[master_df["Client Code"] == scheme_code].copy()
    if table.empty:
        return [], {}
...
def resolve_xls_to_xlsx_for_reading(xls_file, output_dir):
    """Return an xlsx path for an xls/xlsx yield master file."""
    ...
    if os.path.exists(xls_file):
        source_path = xls_file
    else:
        source_path = os.path.join(".", xls_file)

    # Try pandas/xlrd directly first in load_yield_master_lookup. If xlrd is not installed,
    # this converter lets the script still read the files as xlsx.
    libreoffice_commands = ["libreoffice", "soffice"]
    for command in libreoffice_commands:
        try:
            subprocess.run(
...
def read_yield_file(file_path):
    """Read one yield master file and return ISIN -> Aggregated Yield records."""
    records = []
...
def load_yield_master_lookup(required=True):
    """Load all yield master files, keyed by ISIN.

    If required=True, do not silently continue when yield master files are missing.
    Debt schemes need these files for the Yield comparison, so missing files must stop the run.
    """
    lookup = {}
...
def debt_yield_for_compare(value):
    """Debt Yield comparison rule: raw yield * 100, then ROUND_HALF_UP to 2 decimals."""
    dec_value = to_decimal(value)
    if dec_value is None:
        return None
...
def compare_debt_scheme(ws, scheme_code, master_df, yield_lookup, all_discrepancies):
    ...
    for isin in all_isins:
        ...
        for col in DEBT_COMPARE_COLS:
            if col == "Yield":
                master_value = yield_master_row.get("Aggregated Yield") if yield_master_row else None
                ...
                if yield_master_row is None:
                    add_discrepancy(
                        ...
                        "Yield master does not have this ISIN",
                        sebi_row.get("Excel Row"),
                    )
