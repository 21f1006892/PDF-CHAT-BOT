...
OUTPUT_FILE = "SEBI_Monthly_Portfolio_Equity_First_Total_Reconciled_v4.xlsx"

# IMPORTANT:
# In the uploaded master file, Total Market Value (Rs.) matches SEBI's
# "Market/Fair Value\n(Rs.in Lacs)" after dividing by 100000 because 1 lac = 100000.
# If your local master file is already in thousands, change this to 1000.
MARKET_VALUE_DIVISOR = 100000

# Numeric tolerance avoids false mismatch due to tiny Excel float differences.
...
def same_value(master_value, sebi_value, column_name):
    mv = to_number(master_value)
    sv = to_number(sebi_value)

    if mv is not None or sv is not None:
        if mv is None or sv is None:
            return False
        tolerance = AMOUNT_TOLERANCE if "Market/Fair" in column_name else PERCENT_TOLERANCE
        if column_name == "Quantity":
            tolerance = 0
        return abs(mv - sv) <= tolerance
...
def read_master_equity(master_df, scheme_code):
    ...
    master_table["Market/Fair Value\n(Rs.in Lacs)"] = (
        pd.to_numeric(master_table["Market/Fair Value\n(Rs.in Lacs)"], errors="coerce")
        / MARKET_VALUE_DIVISOR
    ).round(2)

    # Master stores % as 1.23 for 1.23%, while SEBI sheet stores it as 0.0123.
    # Divide by 100 so field-level and total % comparisons match SEBI format.
    master_table["% to Net Assets"] = (
        pd.to_numeric(master_table["% to Net Assets"], errors="coerce") / 100
    )
...
def write_index_output(wb, discrepancies):
    ...
    title_cell.value = "Equity Reconciliation Output - First Total Section Only"
